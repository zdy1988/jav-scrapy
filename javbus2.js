...
function find(page_index, callback) {
  var sql = "SELECT * FROM jav WHERE error IS NOT NULL ORDER BY MovieID LIMIT ?,?";
...
function save(item) {
  var sql = "update jav set Title = ?, Description =?, Pubdate = ?, Actor = ?, Duration = ?, Tags = ?, Error=?, Director=?, Producer=?, Publisher=?, Series=? where ID = ?";
