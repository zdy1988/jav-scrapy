...
var website = require("./www.javlibrary.com.js");

const version = require("./package.json").version;

program.version(version).usage("[options]").option("-i, --index <num>", "设置起始页数，默认值：1", 1).parse(process.argv);
...
function find(page_index, callback) {
  var sql = "SELECT * FROM movie WHERE pubdate IS NULL ORDER BY MovieID LIMIT ?,?";
...
function save(item) {
  var sql = "update movie set Title = ?, Description =?, Pubdate = ?, Actor = ?, Duration = ?, Tags = ?, Error=?, Director=?, Producer=?, Publisher=?, Series=? where ID = ?";
...
function handleItem(item, index, callback) {
  let url = website.url + "vl_searchbyid.php?keyword=" + item.MovieID;
